...
def init_group_names():
    return[ ("(1) MISC1",{'layout': 'bsp'}),
            ...
            ("(4) CHAT",{'layout': 'ratiotile'}),
            ("(5) MUSIC",{'layout': 'monadtall'}),
            ("(6) GAME",{'layout': 'max'}),
            ("(7) WATCH",{'layout': 'tile'}),
            ("(8) SCHL1",{'layout': 'max'}),
            ("(9) SCHL2",{'layout': 'max'})]
...
for i, (name, kwargs) in enumerate(group_names, 1):
    keys.append(Key(["mod1" or "mod5"], str(i), lazy.group[name].toscreen()))        # Switch to another group
    keys.append(Key(["mod1" or "mod5", "shift"], str(i), lazy.window.togroup(name))) # Send current window to another group
